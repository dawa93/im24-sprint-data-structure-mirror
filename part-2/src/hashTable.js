...
class HashTable {
  ...
  _resize(newLimit) {
    // this._storage = LimitedArray(newLimit); // newLimit === 16
    let originalStorage = this._storage;
    let originalLimit = this._limit;

    this._size = 0;
    this._limit = newLimit;
    this._storage = LimitedArray(this._limit);

    for (let i = 0; i < originalLimit; i++) {
      let oldBucket = originalStorage.get(i);
      if (oldBucket) {
        for (let j = 0; j < oldBucket.length; j++) {
          this.insert(oldBucket[j][0], oldBucket[j][1]);
        }
